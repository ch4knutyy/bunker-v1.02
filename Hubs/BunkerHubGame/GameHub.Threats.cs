...
namespace Bunker.Hubs
{
	public partial class GameHub
	{
		...
		public async Task RollThreatSupportDice()
		{
			...
			var activePlayers = GetActiveThreatPlayers(room);
		...
		public async Task ResolveCurrentThreat()
		{
			...
			var protectedVolunteer = hasVolunteer && threatState.Contributions.Any(c =>
				...
				(HasAny(c.TagsSnapshot, GetRadiationProtectionTags()) || HasAny(c.TagsSnapshot, ["decontamination_area"])));
		...
		public async Task SubmitThreatMiniGameAnswer(string questionId, string optionId, string? language = null)
		{
			...
			{
				await FinalizeRadiationOperationAsync(room, context.RoomId, threatState, miniGame, NormalizeThreatLanguage(language));
				await NotifyReturnedThreatItems(room, threatState);
				return;
			}
		...
		private async Task NotifyReturnedThreatItems(Room room, ThreatInteractionState threatState)
		{
		...
		private static ThreatInteractionState EnsureRadiationThreatState(Room room)
		{
		...
		private static ThreatOperationBonusState BuildRadiationOperationBonuses(ThreatInteractionState threatState)
		{
			...
			foreach (var contribution in threatState.Contributions.Where(IsActiveThreatContribution))
			{
				...
				{
					if (HasAny(tags, ["engineering_general", "electrical_repair", "mechanical_repair"]))
					{
					...
					if (HasAny(tags, ["radiation_analysis"]))
					{
						autoCategories.Add("detection");
						contributionUseful = true;
					}

					if (HasAny(tags, ["chemistry"]))
					{
						bonuses.HintTokens++;
						contributionUseful = true;
					}

					if (HasAny(tags, ["medical_general"]))
					{
				...
				{
					if (HasAny(tags, ["radiation_detection"]))
					{
					...
					if (HasAny(tags, ["radiation_protection"]))
					{
					...
					if (HasAny(tags, ["sealant", "repair_tools"]))
					{
					...
					if (HasAny(tags, ["medical_supplies"]))
					{
						bonuses.MedicalMitigationCount++;
						contributionUseful = true;
					}
			...
			bonuses.AutoCompletedCategories = [.. autoCategories];
			bonuses.AutoCompletedTaskCount = autoCategories.Count;
			bonuses.UsefulContributionIds = [.. usefulContributionIds];
			return bonuses;
		}

		private static bool IsRadiationThreatActive(Room room, ThreatInteractionState threatState) =>
		...
		private static bool CanCollectThreatContributions(Room room, ThreatInteractionState threatState) =>
		...
		private static List<KeyValuePair<string, Player>> GetActiveThreatPlayers(Room room) =>
		...
		private Item? CreateThreatSupportItem(Room room)
		{
			...
			return new Item
			{
				...
				ProtectionTags = [.. data.ProtectionTags],
		...
		private static PlanChoiceScoreRequest BuildPlanChoiceScoreRequest(Room room, ThreatInteractionState state, JsonElement plan)
		{
		...
		private bool ApplyAirFilterPlanEffects(Room room, ThreatInteractionState state, JsonElement plan, string outcome)
		{
			...
			{
				state.OperationBonuses.UsefulContributionIds =
					[.. state.Contributions
			...
			foreach (var effect in list.EnumerateArray().Where(effect => GetJsonString(effect, "", "type") == "add_physical_condition"))
			{
				...
				List<string> targets = target == "all_active_players"
					? [.. GetActiveThreatPlayers(room).Select(entry => RoomService.GetPlayerKey(entry.Value))]
					: [.. GetThreatParticipantIds(state).Where(id => !IsPlanChoiceParticipantProtected(state, plan, id)).Take(1)];
		...
		private bool ApplyAirFilterPhysicalCondition(Room room, string playerId, string effectConditionId, string severityCode, string mergeRule)
		{
			...
			player.AdditionalConditionEffects.Add(new PlayerConditionEffect
			{
				...
				Name = SeverityHelper.FormatNameWithSeverity(condition.Name, level, "uk"),
				SeverityCode = severityCode,
				SeverityLevel = SeverityHelper.GetSeverityName(level, "uk"),
				SourceThreatId = AirFilterFailureThreatId,
		...
		private static List<IReadOnlyList<string>> GetJsonStringGroups(JsonElement? root, params string[] path) =>
			GetJsonElement(root, path) is { ValueKind: JsonValueKind.Array } value
				? [.. value.EnumerateArray()
					...
					.Select(group => (IReadOnlyList<string>)[.. group.EnumerateArray()
						.Where(item => item.ValueKind == JsonValueKind.String)
						.Select(item => item.GetString() ?? "")
						.Where(item => item.Length > 0)])]
				: [];

		private static void AddThreatContribution(
			Room room,
			ThreatInteractionState threatState,
			string sourceType,
			string sourceId,
			...
			string displayName)
		{
			...
			threatState.Contributions.Add(new ThreatContributionState
			{
				...
				TagsSnapshot = [.. tags.Distinct(StringComparer.OrdinalIgnoreCase)],
				DisplayName = displayName
			});
		}

		private object BuildThreatPublicState(Room room, string? viewerPlayerId = null)
		{
			var threatState = EnsureRadiationThreatState(room);
			if (!IsRadiationThreatActive(room, threatState))
			{
			...
			List<object> viewerContributions = [];
			if (!string.IsNullOrWhiteSpace(viewerPlayerId))
			{
				viewerContributions = [.. threatState.Contributions
			...
			List<object> participantViews = [.. participantIds.Select(id =>
			{
		...
		private static List<string> GetRadiationSolverTags() =>
			["radiation_analysis", "chemistry", "engineering_general", "medical_general"];

		private static List<string> GetRadiationResourceTags() =>
			["radiation_detection", "sealant", "repair_tools", "decontamination_supplies"];

		private static List<string> GetRadiationFacilityTags() =>
			["laboratory", "decontamination_area", "sealed_zone", "control_room"];
		...
		private static string GetMitigatedRadiationSeverity(string baseSeverity, ThreatInteractionState threatState)
		{
		...
		private static List<string> GetConditionLanguageOrder(
