...
public partial class GameHub
{
    ...
    public async Task StartGameFromLobby(string previewToken, bool confirmation, string commandId)
    {
        ...
        var readinessOverrideUsed = false;
        lock (room.GameSettingsSyncRoot)
        {
            ...
            readinessOverrideUsed = (settings.HostCanStartWithoutAllReady || settings.ReadyRequirement == ReadyRequirementMode.HostDecision) &&
                RoomService.GetGameplayPlayersSnapshot(room).Any(entry => entry.Value.IsConnected && !entry.Value.IsLobbyReady);
        ...
        await AppendGmAudit(room, RoomService.GetPlayerKey(host), "game_started_from_lobby", GmAuditResult.Success,
            readinessOverrideUsed ? "Game started after an explicit host readiness override." : "Game started after lobby validation.",
            commandId: commandId, allowUndo: false);
    }
