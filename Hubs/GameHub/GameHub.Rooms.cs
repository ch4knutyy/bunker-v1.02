    ...
    public partial class GameHub
    {
		...
		private object? BuildVotingReconnectInfo(Room room, Player player)
		{
			...
			return new
			{
				...
				results = voting.VoteCounts.Select(kv => new
				{
					connectionId = _roomService.GetCurrentConnectionId(room, kv.Key) ?? kv.Key,
					playerName = _roomService.GetPlayerByAnyId(room, kv.Key)?.Name ?? "Unknown",
					voteCount = kv.Value
				}).OrderByDescending(r => r.voteCount).ToList(),
				topVotedPlayerId = voting.TopVotedPlayerId != null
					? _roomService.GetCurrentConnectionId(room, voting.TopVotedPlayerId) ?? voting.TopVotedPlayerId
					: null,
				topVotedPlayerName = voting.TopVotedPlayerId != null
					? _roomService.GetPlayerByAnyId(room, voting.TopVotedPlayerId)?.Name
					: null,
				isTie = voting.IsTie,
				...
				votes = voting.State == VotingState.Active ? null : voting.Votes.Select(v => new
				{
					voterId = _roomService.GetCurrentConnectionId(room, v.Key) ?? v.Key,
					voterName = _roomService.GetPlayerByAnyId(room, v.Key)?.Name ?? "Unknown",
					targetId = _roomService.GetCurrentConnectionId(room, v.Value) ?? v.Value,
					targetName = _roomService.GetPlayerByAnyId(room, v.Value)?.Name ?? "Unknown"
				})
			};
		}
