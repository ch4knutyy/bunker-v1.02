...
test('ambient root, theme switch, reaction dedupe and reset are idempotent', async ({ page }) => {
  ...
  await expect(page.locator('body')).toHaveClass(/apocalypse-reaction-apocalypse-reveal/);
...
test('effect activation is generic, duplicate-safe, and timer states are canonical', async ({ page }) => {
  ...
  await expect(page.locator('body')).toHaveClass(/apocalypse-reaction-apocalypse-effect/);
...
test('ambient stays non-interactive without horizontal overflow on mobile and ultrawide', async ({ page }) => {
  await page.goto('/Bunker');
...
test('visual acceptance fixtures for three representative themes', async ({ page }, testInfo) => {
  ...
  await page.goto('/Bunker');
  for (const theme of ['wasteland-olive', 'biohazard-green', 'machine-cyan']) {
    await page.evaluate(apocalypse => {
