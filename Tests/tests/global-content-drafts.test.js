...
const service = fs.readFileSync('Services/Bunker/Content/Global/GlobalContentDraftService.cs', 'utf8');
...
test('draft lifecycle is in-memory and exposes no commit or filesystem writer', () => {
  ...
  assert.doesNotMatch(service, /File\.(Write|Move|Copy|Delete)|AtomicReplace/);
...
test('draft UI has guarded actions and no commit controls', () => {
  ...
  assert.doesNotMatch(section, /Save to file|Hot reload/i);
