...
test('effects are guarded and applied once before fresh room snapshot', () => {
  ...
  const body = hub.slice(start, hub.indexOf('private PlanChoiceScoreRequest', start));
  assert.match(body, /if \(state\.Resolution\.EffectsApplied\)/);
  assert.ok(body.indexOf('ApplyAirFilterPlanEffects') < body.indexOf('EffectsApplied = true'));
  assert.ok(body.indexOf('EffectsApplied = true') < body.lastIndexOf('BroadcastThreatState'));
});
