...
const lobbyHelper = fs.readFileSync('wwwroot/js/bunker/apocalypse-category-effects.js', 'utf8');
...
test('category effects use the single canonical controller and timer owners', () => {
  ...
  assert.doesNotMatch(game + lobbyHelper, /apocalypseCategoryEventTimer|__categoryEffectsWrapped|installAmbientOverride|installRenderHooks/);
  assert.doesNotMatch(lobbyHelper, /global\.(?:triggerApocalypseAmbientEvent|clearApocalypseAmbientEvent|renderApocalypse)\s*=/);
});
...
test('canonical event selection is explicit, allowlisted and weighted 65/35', () => {
  ...
  assert.match(game, /modifierIds[\s\S]*\.filter\(value => modifiers\.has\(value\)\)[\s\S]*\.slice\(0, maximum\)/);
